section .data
    ...
    prompt_sensor db "Enter water level (0-100):", 0
...
_start:
    ; Prompt user to enter sensor value
...
process_sensor:
    ...
    cmp ax, 90               ; Compare to high threshold
    jg stop_motor        ; If greater than 90, alarm remains on but motor turned off

    ; Moderate level: motor on and alarm turned on
    jmp start_alarm

activate_motor:
    mov byte [motor_status], 1 ; Turn motor ON
    mov byte [alarm_status], 0 ; Ensure alarm is OFF
    jmp display_low_status

stop_motor:
    mov byte [alarm_status], 1 ; Trigger alarm
    jmp display_high_status

start_alarm:
    mov byte [motor_status], 1 ; Motor remains ON
    mov byte [alarm_status], 1 ; Ensure alarm is ON
